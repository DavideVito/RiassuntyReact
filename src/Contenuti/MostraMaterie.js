...
import { getAnniMaterieAndRiassunti } from "../FirebaseStuff/db";
...
function MostraMaterie(props) {
  ...
  const fetchAnni = () => {
    ...
    async function prendiAnni() {
      ...
      console.log(
        "AnniLocal",
        JSON.parse(localStorage.getItem("indirizzi"))[0].dati
      );
    ...
    let indirizzo = sessionStorage.getItem("in");

    if (!indirizzo) window.location.href = "/";

    getAnniMaterieAndRiassunti(idMateria, indirizzo).then((ann) => {
      cambiaAnni(ann);
